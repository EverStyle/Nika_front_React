...
export default function FileInput({ name }) {
   ...
   return (
      <div className={fileInput.fileInput}>
         <div className={fileInput.hint}>Загрузите реквизиты (файл в формате .PDF, размер не более 2 Мб.)</div>
         {file ?
            <div className={fileInput.fileName}>
               {`${file.name} (${(file.size / 1024 / 1024).toFixed(1)} Мб.)`}
               <span className={fileInput.removeBtn} onClick={() => { setFile(null); inputElement.current.value = ''; }}>
            ...
            :
            <label className={fileInput.label} htmlFor={name}>
               Выбрать файл
            </label>
         }
